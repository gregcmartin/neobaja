...
const int16_t baja_band_dy[BAJA_ROAD_BANDS + 1] = {
    2, 3, 4, 6, 8, 10, 13, 17, 22, 28, 37, 48, 60, 71, 82, 94, 106, 117, 128,
    140
};
...
const uint8_t baja_band_stripe_shift[BAJA_ROAD_BANDS] = {
    23, 22, 22, 21, 20, 20, 20, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19,
    19
};

const int16_t baja_band_half_width[BAJA_ROAD_BANDS] = {
    3, 5, 7, 9, 12, 15, 20, 26, 33, 43, 57, 72, 87, 102, 117, 133, 149, 163,
    179
};

/* depth = camera_height * focal / dy_mid */
static const BajaFp band_depth_fp[BAJA_ROAD_BANDS] = {
    12582912, 8987794, 6291456, 4493897, 3495253, 2735416, 2097152, 1613194,
    1258291, 967916, 740171, 582542, 480264, 411206, 357469, 314573, 282128,
    256794, 234756
};

/* screen pixels per world unit at the middle of each band */
static const BajaFp band_scale_fp[BAJA_ROAD_BANDS] = {
    54613, 76459, 109227, 152917, 196608, 251221, 327680, 425984, 546133,
    709973, 928427, 1179648, 1430869, 1671168, 1922389, 2184533, 2435755,
    2676053, 2927275
};

/* screen pixels per world unit, and the depth, at each band boundary */
static const BajaFp band_edge_scale_fp[BAJA_ROAD_BANDS + 1] = {
    39719, 63716, 89951, 127431, 172032, 220585, 284402, 370421, 478634,
    617368, 804636, 1039068, 1293171, 1541711, 1787997, 2045095, 2303314,
    2550256, 2796032, 3252527
};

static const BajaFp band_edge_depth_fp[BAJA_ROAD_BANDS + 1] = {
    17301504, 10785353, 7639625, 5392677, 3994575, 3115334, 2416284, 1855173,
    1435743, 1113104, 854044, 661357, 531403, 445735, 384338, 336021, 298350,
    269461, 245775, 211280
};
